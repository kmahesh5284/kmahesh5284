...
{
	printk(BIOS_DEBUG,"USER_INFO: pmc_enable_mask value is:%d\n",mask);
	uint32_t pm1_cnt = pmc_read_pm1_control();
	printk(BIOS_DEBUG,"USER_INFO: pm1cnt var value before bit-setmasking is:%d\n",pm1_cnt);
	pm1_cnt |= mask;
	printk(BIOS_DEBUG,"USER_INFO: pm1cnt var value after bit-set masking is:%d\n",pm1_cnt);
	pmc_write_pm1_control(pm1_cnt);
}

void pmc_disable_pm1_control(uint32_t mask)
{
	printk(BIOS_DEBUG,"USER_INFO: pmc_disable_mask value is:%d\n",mask);
	uint32_t pm1_cnt = pmc_read_pm1_control();
        printk(BIOS_DEBUG,"USER_INFO: pm1cnt var value before bit-clear masking is:%d\n",pm1_cnt);
	pm1_cnt &= ~mask;
        printk(BIOS_DEBUG,"USER_INFO: pm1cnt var value after bit-clear masking is:%d\n",pm1_cnt);
	pmc_write_pm1_control(pm1_cnt);
}
