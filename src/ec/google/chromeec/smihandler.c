...
{	
	int scm = sci_mask;
	int smm = smi_mask;
	printk(BIOS_DEBUG,"USER_INFO: The value of sci_mask is:%d\n",scm);
	printk(BIOS_DEBUG,"USER_INFO: The value of smi_mask is:%d\n",smm);

	switch (apmc) {
	case APM_CNT_ACPI_ENABLE:
		printk(BIOS_DEBUG, "Userinfo_APMC_EN_value is %x",APM_CNT_ACPI_ENABLE);
		google_chromeec_set_smi_mask(0);
		clear_pending_events();
